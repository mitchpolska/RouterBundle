...
namespace ITZ\RouterBundle\Component\Routing\Generator;
...
class UrlGenerator implements UrlGeneratorInterface
{
    ...
    protected function doGenerate($variables, $defaults, $requirements, $tokens, $parameters, $name, $absolute)
    {
        ...
        if (isset($requirements['_host']) && '' != $requirements['_host'])
        {
            $host = $requirements['_host'];
            if (false !== strpos($host, '{'))
            {
                $subdomainVarRegExp = '/\{([a-zA-Z\-]+)\}/';
                $paramsTmp = array_merge($parameters, array('http://' => '', 'https://' => ''));
                preg_match_all($subdomainVarRegExp, $host, $subdomainMatches, true);
                $subdomainVar = $subdomainMatches[0][0];
                $host = str_replace(array_keys($paramsTmp),array_values($paramsTmp),$host);
                unset($parameters[$subdomainVar]);
            }
        }
