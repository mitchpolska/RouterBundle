...
namespace ITZ\RouterBundle\Component\Routing\Matcher\Dumper;
...
EOF;
    ...
    private function compileRoute(Route $route, $name, $supportsRedirections, $parentPrefix = null)
    {
        ...
        # Subdomain section
        $hostIsRequire   = false;                           # check if host is require in current route
        $subdomainVar    = '';
        $requirementHost = $route->getRequirement('_host'); # assign variable _host requirement
        $requirementSubdomainRegExp = '';
        if ($requirementHost && '' != $requirementHost)     # if host is require
        {
            $hostIsRequire   = true;
            if (false !== strpos($requirementHost, '}'))
            {
                $subdomainVarRegExp = '/\{([a-zA-Z\-]+)\}/';
                preg_match_all($subdomainVarRegExp, $requirementHost, $subdomainMatches, true);
                $subdomainVar = $subdomainMatches[1][0];
                $requirementSubdomainRegExp = $route->getRequirement($subdomainVar);
            }
        }
        # End subdomain section

        if (!count($compiledRoute->getVariables()) && false !== preg_match('#^(.)\^(?P<url>.*?)\$\1#', str_replace(array("\n", ' '), '', $compiledRoute->getRegex()), $m)) {
            # Subdomain section
            if ('' != $requirementHost)
            {
                if ('' != $requirementSubdomainRegExp)
                {
                    $conditions[] = sprintf('preg_match("%s", strstr($_SERVER["SERVER_NAME"], ".", true))', $requirementSubdomainRegExp);
                }
                else
                {
                    $conditions[] = sprintf('$_SERVER["SERVER_NAME"] == "%s"', $requirementHost);
                }
            }
            # End subdomain section
